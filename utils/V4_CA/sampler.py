...
from model.V4_CA.gaussian_diffusion import extract_into_tensor
...
class SpacedSampler(nn.Module):
    ...
    def predict_noise_tiled(
        ...
        for (hi_latent, hi_end_latent, wi_latent, wi_end_latent), (hi_img, hi_end_img, wi_img, wi_end_img) in zip(tiles_latent, tiles_img):
            ...
            if fix_init_noise:
                ...
                tile_x = x[:, :, hi_latent:hi_end_latent, wi_latent:wi_end_latent]
            tile_cond = {
                "c_lq2": cond["c_lq2"][:, :, hi_latent:hi_end_latent, wi_latent:wi_end_latent],
                "c_lq1_mscn_norm": cond["c_lq1_mscn_norm"][:, :, hi_img:hi_end_img, wi_img:wi_end_img],
                "c_lq1_color": cond["c_lq1_color"][:, :, hi_img:hi_end_img, wi_img:wi_end_img],
                "c_txt": cond["c_txt"]
            }
            if uncond:
                # not implemented
            ...
            # accumulate noise
            eps[:, :, hi_latent:hi_end_latent, wi_latent:wi_end_latent] += tile_eps * weights
            count[:, :, hi_latent:hi_end_latent, wi_latent:wi_end_latent] += weights
    ...
    def p_sample(
        ...
        if tiled:
            eps = self.predict_noise_tiled(model, x, t, cond, uncond, cfg_scale, tile_size, tile_stride, fix_init_noise)
        else:
        ...
        if cond_fn:
            ...
            assert x.shape[0] == 1, "guidance sampling currently only support batch size = 1"
    ...
    def sample(
        ...
        for i, step in enumerate(iterator):
            ...
            img = self.p_sample(
                ...
                tiled, tile_size, tile_stride, fix_init_noise=True if i == 0 else False
            )
